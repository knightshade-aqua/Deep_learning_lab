...
from utils import check_dir, set_random_seed, accuracy, mIoU, get_logger, get_params_groups
...
from data.segmentation import DataReaderSingleClassSemanticSegmentationVector, DataReaderSemanticSegmentationVector, selected_class_ids
...
def parse_arguments():
    ...
    parser.add_argument('--pretrained_model_path', type=str, default='models/ckpt_best.pth')
    parser.add_argument('--output-root', type=str, default='results_single_test')
...
def main(args):
    ...
    model = AttSegmentator(num_classes, pretrained_model.features, args.att, img_size).cuda()
...
def validate(loader, model, criterion, log, logger, epoch=0):
    ...
    results = {"loss": loss_meter.mean, "miou": iou_meter.mean, "miou_std": iou_meter.std}
