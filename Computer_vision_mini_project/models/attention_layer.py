...
class ScaleDotProdAttention(nn.Module):
    def __init__(self, encoder_dim, att_size=512):
        ...
        #self.W_K = nn.Linear(encoder_dim, 512, bias=False)
        #self.W_V = nn.Linear(encoder_dim, 1, bias=False)
    ...
    def forward(self, encoder_output, cls_vector):
        ...
        #value = (encoder_output * self.W_V(encoder_output))
        ...
        att = torch.matmul(query,torch.transpose(key,1,2)).squeeze()
        att_scaled = self.scale_score * att
        # att = 
        # att_scaled = 
