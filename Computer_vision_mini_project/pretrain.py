...
def parse_arguments():
    ...
    parser.add_argument('--output-root', type=str, default='results_ex1')
...
def main(args):
    ...
    train_data = DataReaderPlainImg(os.path.join(data_root, "images/256/train"), transform=train_transform)
    if args.dataset_size is not None:
        train_data = torch.utils.data.Subset(train_data, range(args.dataset_size))
    val_data = DataReaderPlainImg(os.path.join(data_root, "images/256/val"), transform=val_transform)
...
def train(data_loader, t_model, s_model, criterion, optimizer, lr_schedule, wd_schedule, momentum_schedule, epoch, fp16_scaler, log=None):
    ...
    for iteration, images in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            output_teacher_train = t_model(images[:2])
            output_student_train = s_model(images)
            loss = criterion(output_student_train, output_teacher_train, epoch)  
            #raise NotImplementedError("TODO: load weight initialization")
        ...
        with torch.no_grad():          
            ...
            for s_param, t_param in zip(s_model.parameters(), t_model.parameters()):
                t_param.data.mul_(m).add_((1 - m) * s_param.detach().data)
            #raise NotImplementedError("TODO: load weight initialization")
...
def validate(loader, t_model, s_model, criterion, epoch, args, log=None):
   ...
   #T-SNE Embeddings
    feature_maps_embedded = TSNE(n_components=2, random_state=42).fit_transform(feature_maps)
